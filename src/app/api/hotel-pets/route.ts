...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!ownerId || isNaN(parseInt(ownerId)))
      return NextResponse.json({ error: 'Parameter ownerId wajib diisi.' }, { status: 400 });

    const uid = await odooLogin();
    const pets = await odooExecuteKw(uid, 'x_pets', 'search_read',
      [[['x_studio_pet_owner_2', '=', parseInt(ownerId)]]],
      { fields: ['id', 'x_name', 'x_studio_jenis_pet', 'x_studio_berat', 'x_studio_ras_pet_1', 'x_studio_gender', 'x_studio_kategori_berat'], order: 'x_name asc' }
    ) as Array<{ id: number; x_name: string; x_studio_jenis_pet: string; x_studio_berat: number; x_studio_ras_pet_1: [number, string] | false; x_studio_gender: string; x_studio_kategori_berat: string }>;

    return NextResponse.json({ results: pets.map((p) => ({
      id: p.id, name: p.x_name, jenis: p.x_studio_jenis_pet, berat: p.x_studio_berat,
      ras: p.x_studio_ras_pet_1 ? p.x_studio_ras_pet_1[1] : '—',
      gender: p.x_studio_gender, kategoriBerat: p.x_studio_kategori_berat,
    })) });
  } catch (err) {
